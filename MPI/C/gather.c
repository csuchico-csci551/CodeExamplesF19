...
int main(int argc, char** argv) {
  ...
  int *rec_data = NULL;
  if (world_rank == 0) {
    rec_data = (int *)malloc(sizeof(int) * num_elements_per_proc * world_size);
    assert(rec_data != NULL);
  }
  MPI_Gather(sub_rand_nums, num_elements_per_proc, MPI_INT, rec_data, num_elements_per_proc, MPI_INT, 0, MPI_COMM_WORLD);

  if (world_rank == 0){
    for (int i= 0 ; i < num_elements_per_proc*world_size ; ++i){
      printf("Process 0 received number %d from processes\n",rec_data[i]);
    }
  }
